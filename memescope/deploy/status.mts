...
//
// Голый счётчик заставляет лезть в базу руками, чтобы узнать, сбой это или
// единичный отказ внешнего источника. 20 сентября счётчик впервые за недели
// стал ненулевым — и по отчёту нельзя было сказать ничего, кроме «три».
// Печатаем последние пять: действие, время и начало подробностей.
if (errors24h > 0) {
  const errs = await prisma.auditLog.findMany({
    where: { action: { contains: "error" }, createdAt: { gte: dayAgo } },
    orderBy: { createdAt: "desc" },
    take: 5,
    select: { action: true, createdAt: true, details: true },
  });
  for (const e of errs) {
    // Подробности — сырой JSON произвольной длины; в отчёт идёт только начало,
    // остальное читается в базе по действию и времени.
    const raw = (e.details ?? "").replace(/\s+/g, " ");
    // Код Prisma (P2024, P1008, P2002…) стоит в конце длинного сообщения и в
    // обрезку не попадал — а именно он отличает состязание за базу от ошибки
    // в самом запросе. Вытаскиваем его отдельно и ставим в начало.
    const code = raw.match(/\bP\d{4}\b/)?.[0];
    const d = raw.slice(0, 160);
    lines.push(`  · ${e.action} (${ago(e.createdAt)})${code ? ` [${code}]` : ""}${d ? `: ${d}` : ""}`);
  }
}
...
// итоговая цифра тихо смешивает посчитанное с придуманным.
{
  ...
  // ему чужой убыток или чужую прибыль.
  {
    ...
    for (const [rule, title] of [
      ...
      // ГДЕ ИМЕННО ИЗМЕРЕНИЕ ЕЩЁ НЕ РАБОТАЕТ.
      // Чтение резервов пула (PR #106) закрыло устаревшие цены на EVM-сетях,
      // но Solana и Robinhood Chain остались непокрытыми: у них другая модель
      // пулов. Общий счётчик «закрыто по устаревшей цене» этого не показывает,
      // потому что мешает старые сделки с новыми. 28 августа достоверная часть
      // лотерейного трека перестала расти вовсе, хотя закрытий прибавлялось, —
      // по общей цифре понять причину было нельзя, пришлось бы гадать.
      // Печатаем разрез последних суток по сетям: он отвечает на вопрос
      // «пробел в покрытии или токены просто умирают» одним взглядом.
      const dayAgo = Date.now() - 24 * 60 * 60 * 1000;
      const fresh = c.filter((p) => (p.closedAt?.getTime() ?? 0) >= dayAgo);
      if (fresh.length) {
        const freshShaky = fresh.filter((p) => p.closeReason?.includes("НЕДОСТОВЕРЕН"));
        const byChain = new Map<string, number>();
        for (const p of freshShaky) byChain.set(p.token.chain, (byChain.get(p.token.chain) ?? 0) + 1);
        const breakdown = [...byChain.entries()]
          .sort((a, b) => b[1] - a[1])
          .map(([chain, n]) => `${chain} ${n}`)
          .join(", ");
        lines.push(
          `  · за последние сутки закрыто ${fresh.length}, из них по устаревшей цене ${freshShaky.length}` +
          (breakdown ? ` (${breakdown})` : ""),
        );
        // ПОЧЕМУ ИМЕННО ЭТИ ПУЛЫ НЕ ЧИТАЮТСЯ.
        // Разрез по сетям опроверг мою же догадку: больше всего недостоверных
        // закрытий дала base, а base чтение резервов ПОДДЕРЖИВАЕТ. Значит дело
        // не в сети, а в устройстве конкретных пулов — и отличить одно от
        // другого можно только по адресу, спросив узел напрямую. Печатаем
        // адреса трёх последних: это проверяемый факт, а не ещё одна догадка.
        const latestShaky = [...freshShaky]
      ...
      // РАЗБИВКА ПО СЕТЯМ.
      // Вопрос «а не зарабатывают ли все на такой-то сети» задают по внешним
      // впечатлениям, а отвечать на него надо своими измерениями. Сводная сумма
      // трека этого не показывает: в ней сеть с сотней убыточных сделок и сеть
      // с одним стократником выглядят одинаково. Печатаем по каждой сети число
      // закрытых сделок, прибыльных, итог и долю недостоверных закрытий —
      // последняя нужна, чтобы не принять допущение за результат.
      const byChain = new Map<string, { n: number; wins: number; pnl: number; shaky: number }>();
      for (const p of c) {
        const cur = byChain.get(p.token.chain) ?? { n: 0, wins: 0, pnl: 0, shaky: 0 };
        cur.n += 1;
        if (p.realizedPnlUsd > 0) cur.wins += 1;
        cur.pnl += p.realizedPnlUsd;
        if (p.closeReason?.includes("НЕДОСТОВЕРЕН")) cur.shaky += 1;
        byChain.set(p.token.chain, cur);
      }
      for (const [chain, v] of [...byChain.entries()].sort((a, b) => b[1].n - a[1].n)) {
        lines.push(
          `  · ${chain}: ${v.n} сделок, прибыльных ${v.wins}, итог $${v.pnl.toFixed(2)}` +
          `, по устаревшей цене ${v.shaky}`,
        );
      }
      // КОНЦЕНТРАЦИЯ РЕЗУЛЬТАТА НА ОДНОЙ СДЕЛКЕ.
      // 21 августа трек проверенного правила за полтора часа перешёл из
      // −$71 в +$757: почти весь итог дала ОДНА сделка. По общей сумме это
      // читается как «правило начало работать», хотя это ровно то, о чём
      // предупреждает пре-регистрация: на распределении с толстым хвостом
      // одна сделка двигает среднее на сотни процентов. Поэтому крупнейшая
      // сделка и итог без неё печатаются рядом с итогом всегда — иначе
      // отчёт даёт основание для вывода, которого данные не выдерживают.
      const best = [...c].sort((a, b) => b.realizedPnlUsd - a.realizedPnlUsd)[0];
      if (best && best.realizedPnlUsd > 0) {
        const withoutBest = sum(c) - best.realizedPnlUsd;
        const gain = (best.realizedPnlUsd / (best.costUsd || 1)) * 100;
        // Если выход этой сделки взят по устаревшей цене, весь вклад — не
        // измерение, а допущение. Молчать об этом именно здесь нельзя.
        const shakyBest = best.closeReason?.includes("НЕДОСТОВЕРЕН")
          ? " — ВЫХОД ПО УСТАРЕВШЕЙ ЦЕНЕ, вклад недостоверен"
          : "";
        lines.push(
          `  · крупнейшая сделка трека: ${best.token.symbol} $${best.realizedPnlUsd.toFixed(2)} ` +
          `(${gain.toFixed(0)}% от входа $${best.costUsd.toFixed(2)})${shakyBest}; ` +
          `без неё итог трека $${withoutBest.toFixed(2)} по ${c.length - 1} сделкам`,
        );
        // ПРОВЕРЯЕМОСТЬ. 23 августа одна сделка дала 8329% и в одиночку
        // перевернула трек из −$925 в +$5779. Такую цифру нельзя предъявлять
        // как факт, не дав возможности сверить её с внешним источником:
        // отличить настоящий рост от испорченной цены по самой базе нельзя.
        // Печатаем адрес пары и цены входа/максимума — этого достаточно,
        // чтобы открыть график и убедиться самому.
        lines.push(
          `    сверить: вход $${best.entryPriceUsd.toPrecision(4)}, ` +
    ...
    //
    // Считаются ИЗМЕРЕННЫЕ сделки, а не все закрытые. Пре-регистрация требует
    // 300 сделок «с измеримым исходом», и писалась она 17 августа — до того,
    // как выяснилось, что у части закрытий цена выхода взята из последнего
    // известного снимка, то есть является допущением. 4 сентября трек перешёл
    // отметку 300 закрытых, имея 121 измеренную сделку: по общему счёту
    // предупреждение бы исчезло, и выборка выглядела бы набранной. Считать по
    // допущениям ту проверку, ради которой всё и делалось, нельзя — а ужесточать
    // при обнаружении собственной ошибки измерения правила прямо обязывают.
    const lAll = track("low-liquidity-lottery");
    const lMeasured = lAll.filter((p) => !p.closeReason?.includes("НЕДОСТОВЕРЕН")).length;
    if (lAll.length > 0 && lMeasured < 300)
      lines.push(
        `  · ⚠️ по лотерейному треку измерено ${lMeasured} сделок из 300 минимально нужных ` +
        `(всего закрыто ${lAll.length}) — на хвостовом распределении это ещё не результат`,
      );
