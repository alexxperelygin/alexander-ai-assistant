  ...
  // сначала проверяют, а не празднуют.
  {
    ...
    if (scored.length) {
      ...
      if (top) {
        ...
        log();
        // Раньше здесь стоял готовый вывод «пул сменился, сделка недостоверна».
        // Он печатался всегда — и 17 августа встал под таблицей, в которой
        // адрес пары НЕ менялся ни разу. Заранее написанный вывод, не
        // сверенный с собственными числами, — это и есть выдуманный
        // результат, даже если он написан против себя. Вывод считается.
        const distinct = new Set(pathPairs).size;
        log(`Колонка «Пул» решает вопрос: источник отдаёт по токену ЛУЧШУЮ пару,`);
        log(`и когда ликвидность переезжает в другой пул, котировка скачком`);
        log(`переходит на его цену. В отчёте это выглядит ростом в десятки раз,`);
        log(`но заработать на нём было нельзя: нашей позиции в новом пуле нет.`);
        if (distinct === 0) {
          log(`Здесь адрес пары неизвестен — проверить не на чем, и сделку`);
          log(`следует считать неподтверждённой.`);
        } else if (distinct > 1) {
          log(`Здесь адрес менялся (${distinct} разных пула) — **сделка недостоверна**,`);
          log(`как бы красиво ни выглядело среднее (правило из`);
          log(`\`docs/PREREGISTRATION.md\`).`);
        } else {
          log(`Здесь адрес пары не менялся ни разу, цена росла постепенно через`);
          log(`все наблюдения, а ликвидность росла вместе с ней. По этому признаку`);
          log(`сделка НЕ отбраковывается. Это не доказывает, что на ней можно было`);
          log(`заработать: остаются проскальзывание и исполнимость выхода, которые`);
          log(`проверяются отдельно выше.`);
        }
        log();
      }
...
// ---------- 4g. Частота и размер хвостов ----------
// Вопрос владельца: имеет ли смысл венчурный формат — много мелких потерь
// ради редкого стократного выигрыша. Это не вопрос вкуса, а вопрос двух
// чисел: КАК ЧАСТО случается хвост и КАКОГО он размера. Всё остальное —
// арифметика.
//
// Критерий «среднее без лучшей сделки» для такого формата не годится:
// у венчурного портфеля он отрицателен по определению. Но и «одна сделка
// дала сто крат» ничего не доказывает — по одному наблюдению частоту не
// измерить. Зато её можно измерить на всей неделе: точек входа тысячи.
{
  log(`## 4g. Частота и размер хвостов: есть ли основание у венчурного формата`);
  log();
  log(`Венчурная логика работает, если редкий крупный выигрыш окупает много`);
  log(`мелких потерь. Это проверяется двумя числами — частотой хвоста и его`);
  log(`размером, — а не рассуждением. Считаем их на всей неделе, а не на одной`);
  ...
  // Окно наблюдений — неделя, а заморозка была раньше: со временем «вся
  // неделя» и «после заморозки» сошлись в одну и ту же выборку. Две
  // одинаковые строки в таблице читаются как два независимых подтверждения,
  // хотя это одни и те же сделки, посчитанные дважды. Говорим об этом прямо.
  const sameSample = (groups[0] as { entries: ExitEntry[] }).entries.length ===
    (groups[1] as { entries: ExitEntry[] }).entries.length;
  if (sameSample) {
    log(`⚠️ Окно наблюдений короче срока с заморозки: первые две строки — ОДНА`);
    log(`и та же выборка, а не два независимых среза. Считать их подтверждением`);
    log(`друг друга нельзя.`);
    log();
  }

  log(`| Выборка | Сделок | >+100% | >+500% | >+1000% | >+5000% | Среднее | Среднее без хвостов >10x |`);
  log(`|---|---|---|---|---|---|---|---|`);
  for (const g of groups) {
    const rs = policyReturns(g.entries, { name: "frozen", run: (s, i) => simulateExit(s, i, FROZEN_POLICY) });
    if (rs.length < 20) { log(`| ${g.name} | ${rs.length} | мало данных | | | | | |`); continue; }
    const share = (t: number) => `${rs.filter((r) => r > t).length} (${((rs.filter((r) => r > t).length / rs.length) * 100).toFixed(2)}%)`;
    const body = rs.filter((r) => r <= 10);
    log(`| ${g.name} | ${rs.length} | ${share(THRESHOLDS[0] as number)} | ${share(THRESHOLDS[1] as number)} | ` +
        `${share(THRESHOLDS[2] as number)} | ${share(THRESHOLDS[3] as number)} | ${pct(mean(rs))} | ${pct(mean(body))} |`);
  }
  log();

  // Ключевая арифметика: при каком РЕДЧАЙШЕМ хвосте формат ещё окупается.
  ...
  // она рядом с порогом, венчурный формат держится на удаче, а не на числах.
  {
    ...
    if (bodyMean != null && bodyMean >= 0) {
      ...
      log(`даёт в среднем ${pct(bodyMean)} на сделку, средний хвост — ${pct(tailMean, 0)}.`);
      log(`Значит формат окупается, если хвост случается **не реже чем 1 раз на`);
      log(`${Math.round(breakEvenOneIn)} сделок**. Наблюдаемая частота — 1 на ${observedOneIn.toFixed(0)}.`);
      log();
      const margin = breakEvenOneIn / observedOneIn;
      log(margin >= 10
        ? `Запас ${margin.toFixed(0)}-кратный: даже если истинная частота хвостов в несколько раз хуже наблюдаемой, формат остаётся положительным. Это довод ЗА проверку венчурного формата.`
        : `Запас всего ${margin.toFixed(1)}-кратный. Для распределения, где всё решают единичные события, это не запас: истинная частота легко может оказаться хуже наблюдаемой во столько же раз. Довода ЗА венчурный формат данные не дают.`);
      log();
      log(`Оговорка, без которой таблица вводит в заблуждение: хвостов в выборке`);
      log(`**${tails.length}**. Частота, измеренная по такому числу событий, сама известна`);
      log(`с точностью «в разы». Это оценка порядка величины, а не доказательство.`);
      log();
    }
  }
}
...
// критериев, иначе побеждает тот, который удобнее по факту.
{
  ...
  // Пока выборка не набрана, столбец итога не заполняется ВООБЩЕ — ни
  // галочками, ни крестиками. На 23 сделках «тело +26.8% ✅» и «частота хвостов
  // ❌» одинаково бессмысленны: первое — случайность одной сделки, второе —
  // арифметика (при одном хвостовом событии никакой интервал не может
  // отделиться от единицы). Раскрашивать такое — значит подсказывать вывод там,
  // где вывода нет; сами значения при этом видны и никуда не прячутся.
  const decidableV = rs.length >= MIN_TRADES;
  log(`| Критерий (зафиксирован заранее) | Значение | ${decidableV ? "Итог" : "Пока"} |`);
  log(`|---|---|---|`);
  for (const c of checks)
    log(`| ${c.name} | ${c.detail} | ${!decidableV || c.ok == null ? "рано судить" : c.ok ? "✅ пройден" : "❌ не пройден"} |`);
  log();
...
//
// Вопрос, который задаёт любой, кто увидел в телеграме скриншот «$100 → $360k»:
// почему у него получается, а у нас нет. Отвечать на него рассуждением нельзя —
// только своими данными.
//
// 17 августа выяснилось, что токен из такого скриншота (牛来, рост примерно в
// 3600 раз от капитализации $10.2K) есть в нашей базе, и наш собственный трек
// его КУПИЛ — через трое суток после пампа, по правилу «ликвидность > $50k», и
// закрыл по стопу с минусом $15. Один пример ничего не доказывает, поэтому
// здесь то же самое считается по всем крупным движениям в окне.
//
// Смысл замера: показать, какая часть роста УЖЕ ПРОШЛА к моменту, когда токен
// впервые проходит наш порог ликвидности. Если порог систематически включается
// после основного движения — значит наше правило по построению не может ловить
// такие иксы, и это надо знать про себя, а не узнавать из чужих скриншотов.
{
  const GATE = 50_000;
  log(`## 4i. Крупные движения: где был наш порог, когда они случились`);
  log();
  log(`Берутся токены, выросшие от первого наблюдения минимум в 10 раз. Для`);
  log(`каждого считается, во сколько раз он уже вырос К МОМЕНТУ, когда его`);
  log(`ликвидность впервые превысила $${GATE.toLocaleString("ru")} — то есть когда наше правило`);
  log(`вообще получило право войти.`);
  log();

  const movers: { mult: number; atGate: number | null }[] = [];
  for (const series of byToken.values()) {
    if (series.length < 3) continue;
    const first = series[0] as Snap;
    if (!(first.priceUsd > 0)) continue;
    let peak = first.priceUsd;
    for (const s of series) if (s.priceUsd > peak) peak = s.priceUsd;
    const mult = peak / first.priceUsd;
    if (mult < 10) continue;
    // Момент, когда правило впервые получило право войти.
    let atGate: number | null = null;
    for (const s of series) {
      if ((s.liquidityUsd ?? 0) > GATE) { atGate = s.priceUsd / first.priceUsd; break; }
    }
    movers.push({ mult, atGate });
  }

  if (movers.length < 5) {
    log(`Крупных движений в окне слишком мало (${movers.length}) — считать нечего.`);
    log();
  } else {
    const never = movers.filter((m) => m.atGate == null);
    const gains = movers.filter((m) => m.atGate != null).map((m) => m.atGate as number)
      .sort((a, b) => a - b);
    log(`| Всего движений ≥10x | Ни разу не прошли порог | Прошли порог |`);
    log(`|---|---|---|`);
    log(`| ${movers.length} | ${never.length} | ${gains.length} |`);
    log();
    log(`Вторая колонка — сколько таких движений наше правило не имело права`);
    log(`купить НИКОГДА: ликвидность так и не поднялась выше порога. Это чистая`);
    log(`слепая зона, и устранить её, не трогая порог, нельзя.`);
    log();

    if (gains.length) {
      const q = (f: number) => gains[Math.min(gains.length - 1, Math.floor(gains.length * f))] as number;
      log(`Для тех, что порог всё-таки прошли — во сколько раз токен успел вырасти`);
      log(`ДО того, как мы получили право войти:`);
      log();
      log(`| Квартиль | Рост к моменту, когда порог пройден |`);
      log(`|---|---|`);
      log(`| 25% | ×${q(0.25).toFixed(1)} |`);
      log(`| медиана | ×${q(0.5).toFixed(1)} |`);
      log(`| 75% | ×${q(0.75).toFixed(1)} |`);
      log();
      log(`Доля движений, где к моменту прохождения порога токен уже удвоился:`);
      log(`**${pct(gains.filter((g) => g >= 2).length / gains.length, 0)}**.`);
      log();

      // БЕЗ ЭТОЙ ТАБЛИЦЫ ПРЕДЫДУЩАЯ ВВОДИТ В ЗАБЛУЖДЕНИЕ.
      //
      // «К моменту входа уже прошло ×5.2» само по себе НЕ значит, что мы
      // опоздали: у токена, который вырастет в 3600 раз, после пятикратного
      // роста остаётся ещё почти семисоткратный. Опоздание измеряется не тем,
      // сколько прошло, а тем, сколько ОСТАЛОСЬ. Считаем именно это.
      const left = movers.filter((m) => m.atGate != null)
        .map((m) => m.mult / (m.atGate as number))
        .sort((a, b) => a - b);
      const ql = (f: number) => left[Math.min(left.length - 1, Math.floor(left.length * f))] as number;
      log(`И главное — во сколько раз токен вырос ПОСЛЕ того, как порог пройден,`);
      log(`то есть какая часть движения ещё была доступна нам:`);
      log();
      log(`| Квартиль | Рост, оставшийся после порога |`);
      log(`|---|---|`);
      log(`| 25% | ×${ql(0.25).toFixed(1)} |`);
      log(`| медиана | ×${ql(0.5).toFixed(1)} |`);
      log(`| 75% | ×${ql(0.75).toFixed(1)} |`);
      log();
      log(`Доля движений, где после порога оставалось ещё не меньше 10x:`);
      log(`**${pct(left.filter((g) => g >= 10).length / left.length, 0)}**.`);
      log();
      log(`Если бы здесь стояли единицы, вывод был бы «мы приходим на всё`);
      log(`готовое». Чем эти числа больше, тем меньше опоздание объясняет наш`);
      log(`результат — и тем больше вопросов к тому, что мы делаем ПОСЛЕ входа.`);
      log();
    }

    log(`Как это читать. Порог по ликвидности выбран не из осторожности, а по`);
    log(`измерению: без него медиана свежего токена на 6ч равна −93.7% при 13%`);
    log(`прибыльных, с ним — около −1.3%. Порог покупает выживание. Цена покупки —`);
    log(`ровно та часть роста, которая случилась, пока ликвидность была ниже`);
    log(`порога, плюс движения, до порога не дошедшие вовсе.`);
    log();
    log(`Это НЕ довод снижать порог: по ту сторону порога типичный исход —`);
    log(`потерять почти всё. Это довод понимать, что наша система и охотник за`);
    log(`иксами на капитализации $10k играют в разные игры, и сравнивать их`);
