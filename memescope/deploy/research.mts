  ...
  // сначала проверяют, а не празднуют.
  {
    ...
    if (scored.length) {
      ...
      // не стратегия, а частота опроса.
      {
        ...
        log();
        log(`| Частота наблюдений | Сделок | Среднее | Медиана |`);
        log(`|---|---|---|---|`);
        for (const b of buckets)
          log(`| ${b.name} | ${b.rs.length} | ${b.rs.length ? pct(mean(b.rs)) : "—"} | ${b.rs.length ? pct(median(b.rs)) : "—"} |`);
        log();
        log(`Если прибыль сосредоточена в редко опрошенных сделках — измеряется`);
        log(`частота опроса, а не стратегия, и результат недействителен.`);
        log();
      }
    }
  }
