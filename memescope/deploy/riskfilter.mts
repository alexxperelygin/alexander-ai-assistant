...
// доступность данных. Догадку надо заменить числом.
{
  ...
  const duMeas = ctrl.filter((o) => o.status === "DATA_UNAVAILABLE" && o.ret["6h"] != null).length;
  L.push(
    ...
    `и измеримых среди них ${duMeas}.`,
  );
  L.push("");

  // ЧТО ИМЕННО ИСПОРТИЛО СРАВНЕНИЕ.
  //
  // Первая догадка была, что медианы искажены токенами без данных. Она
  // неверна: у DATA_UNAVAILABLE измеримых наблюдений ноль, в медиану они не
  // входят вовсе. Их роль другая и хуже — они раздули знаменатель контрольной
  // группы и тем самым СПРЯТАЛИ разрыв в измеримости, на который поставлен
  // критерий 6. Без них критерий проваливается с большим запасом.
  const avAll = outcomes.filter((o) => o.avoid);
  const avMeasShare = avAll.length ? avAll.filter((o) => o.ret["6h"] != null).length / avAll.length : 0;
  const live = ctrl.filter((o) => o.status !== "DATA_UNAVAILABLE");
  const liveMeasShare = live.length ? live.filter((o) => o.ret["6h"] != null).length / live.length : 0;
  const trueGap = Math.abs(avMeasShare - liveMeasShare);
  L.push(
    `Доля измеримых: AVOID ${(avMeasShare * 100).toFixed(1)}%, ` +
    `контроль без DATA_UNAVAILABLE ${(liveMeasShare * 100).toFixed(1)}% ` +
    `(${live.length} наблюдений). Разрыв **${(trueGap * 100).toFixed(1)} п.п.** при пороге 15.`,
  );
  L.push("");
  L.push(
    trueGap >= 0.15
      ? "⚠️ **Сравнение объясняется выживаемостью наблюдений, а не качеством фильтра.** " +
        "Отбракованные токены после смерти пропадают из котировок и становятся " +
        "неизмеримыми — их провалы в статистику не попадают. Токены с ликвидностью " +
        "выше порога продолжают котироваться даже после обвала, и их провалы " +
        "попадают. Поэтому медиана AVOID выглядит лучше: это выжившие, а не хорошие.\n\n" +
        "Токены со статусом DATA_UNAVAILABLE сами по себе медианы не искажали — " +
        "измеримых среди них ноль. Они сделали хуже: раздули знаменатель контроля и " +
        "спрятали именно этот разрыв, из-за чего критерий 6 на полном периоде прошёл. " +
        "Исключить их из контроля следовало заранее — **ошибка проектирования " +
        "пре-регистрации**. Вердикт NO EDGE остаётся в силе: критерии 2–4 провалены " +
        "независимо от этого, а переигрывать замороженное нельзя."
      : "Разрыв измеримости между группами в пределах порога.",
  );
  L.push("");
}
