...
export function simulateFill(p: FillParams): FillResult {
  ...
  if (p.observedImpactPct != null) {
    ...
    // ПУСТОЙ ПУЛ — ЭТО ИЗМЕРЕНИЕ, А НЕ ОТСУТСТВИЕ ДАННЫХ.
    //
    // Раньше ноль попадал в ветку «ликвидность неизвестна», потому что условие
    // выше требовало > 0. Разница дорогая: неизвестная ликвидность уводит
    // сделку в «неизмеримые», то есть вон из статистики, а пустой пул — это
    // худший из возможных исходов, и он обязан остаться внутри неё полным
    // списанием. Выбрасывать худшие сделки — ровно та выживаемость, против
    // которой построено исследование.
    //
    // 29 августа это стало видно на живых данных: после того как чтение пулов
    // V4 научилось отдавать нулевую активную ликвидность, доля неизмеримых
    // сделок пошла вверх (14 → 17 → 19 за десять часов).
    impactPct = 100;
  } else {
