...
import { FROZEN_EXIT, VENTURE_EXIT } from "./exit-policy";
import { LOTTERY_ENTRY, VALIDATED_ENTRY, type EntryRule } from "../strategy/validated-entry";
...
export async function openPosition(args: OpenPositionArgs) {
  ...
  // Исследовательские треки живут по своим ограничителям. Разбор — ниже.
  const validated = entryRule === "validated-liquidity";
  const lottery = entryRule === "low-liquidity-lottery";
  const research = validated || lottery;
  ...
  if (args.mode === "live" && research)
    throw new Error("Исследовательские треки существуют только на бумаге.");
  ...
  if (research) {
    ...
    // Потолок у каждого трека СВОЙ. Общий потолок означал бы, что треки
    // отбирают слоты друг у друга: более частый вытеснял бы более редкий, и
    // выборка обоих определялась бы не правилом, а очерёдностью.
    const cap = validated ? VALIDATED_ENTRY.maxOpenPositions : LOTTERY_ENTRY.maxOpenPositions;
    const openSame = open.filter((p) => p.entryRule === entryRule);
    if (openSame.length >= cap)
      throw new Error(`Занято ${openSame.length} из ${cap} слотов трека «${entryRule}».`);
  } else {
    ...
    const closedToday = await prisma.position.findMany({
      ...
      where: { closedAt: { gte: today }, OR: [{ entryRule: null }, { entryRule: { notIn: ["validated-liquidity", "low-liquidity-lottery"] } }] },
    });
    ...
    const recent = await prisma.position.findMany({
      where: {
        ...
        OR: [{ entryRule: null }, { entryRule: { notIn: ["validated-liquidity", "low-liquidity-lottery"] } }],
      },
  ...
  // Стоп ставится по политике ТРЕКА. У венчурного трека stopPct совпадает с
  // замороженным, но брать его из другой константы нельзя: если политики
  // разойдутся, позиция откроется с чужим стопом и это никак не проявится.
  const exitPolicy = lottery ? VENTURE_EXIT : FROZEN_EXIT;
  const stopPriceUsd = entryPriceUsd * (1 - exitPolicy.stopPct);
  ...
  const pos = await prisma.position.create({
    data: {
      ...
      events: {
        create: {
          kind: "OPEN",
          message: `${args.mode === "paper" ? "Paper" : "Live (ручная покупка)"} вход${validated ? " по проверенному правилу" : lottery ? " по правилу низкой ликвидности" : ""}: ${quantity.toFixed(2)} шт по $${entryPriceUsd.toPrecision(6)} (комиссии $${feesUsd.toFixed(2)}, impact ${slippagePct.toFixed(2)}%)`,
...
export async function sellPosition(args: SellArgs) {
  ...
  if (pos.entryRule !== "validated-liquidity" && pos.entryRule !== "low-liquidity-lottery")
    await notify(
      pnl >= 0 ? "info" : "warning",
