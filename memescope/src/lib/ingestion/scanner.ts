...
async function maybeOpenValidatedEntry(ctx: {
  ...
  // ТОКЕН, УЖЕ ОТКЛОНЁННЫЙ РАНЬШЕ, БОЛЬШЕ НЕ РАССМАТРИВАЕТСЯ.
  //
  // Замороженное правило говорит: одна сделка на токен, по ПЕРВОМУ подходящему
  // наблюдению. Без этой проверки токен, отклонённый из-за занятых слотов,
  // возвращался в очередь на каждом следующем цикле сканера и мог быть куплен
  // часами позже — а это уже другое правило. Исследование меряло задержку
  // входа отдельно: час задержки съедал около половины результата. То есть
  // повторные попытки тихо превращали живой прогон в проверку входа с
  // задержкой, притом с задержкой случайной длины.
  //
  // Побочно это чинило и счётчик: за сутки набралось 353 записи об отказе при
  ...
  try {
    ...
    // потолка слотов и в какие моменты они пропускались. Запись здесь же
    // закрывает токен навсегда (см. проверку выше): раз в момент первого
    // подходящего наблюдения войти не удалось, войти позже — значит
    // проверять другое правило.
    await prisma.auditLog.create({
      data: {
        actor: "worker",
        action: "validated.entry.skipped",
