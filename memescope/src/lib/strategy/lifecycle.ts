...
export function buildTradePlan(args: {
  ...
  return {
    ...
    // Правила выхода заморожены 8 августа, см. docs/PREREGISTRATION.md.
    // Лесенка тейков убрана не по вкусу, а по измерению: на одних и тех же
    // входах она давала +8.5% против +26.2% у стопа с трейлингом, потому что
    // продавала треть на 1.5x и треть на 2x — то есть обрубала тот самый
    // правый хвост, из которого на мем-коинах и берётся вся прибыль, оставляя
    // убытки целиком. На выборке без фильтра ликвидности она уходила в −4.2%.
    stopCondition:
      `Цена −20% от входа ИЛИ откат 30% от достигнутого максимума ИЛИ ликвидность ` +
      `ниже 60% от уровня на входе ИЛИ sell-маршрут перестал подтверждаться ` +
      `ИЛИ прошло 3 суток → немедленный полный выход.`,
    // Пусто намеренно: частичные фиксации прибыли отключены.
    takeProfitLevels: [],
    fullExitCondition:
      "Полный выход: сработал stop, трейлинг, лимит удержания 3 суток, или статус стал INVALIDATED/EXIT (напр. крупные держатели выходят, объём умирает).",
