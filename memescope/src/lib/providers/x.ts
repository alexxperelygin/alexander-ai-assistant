...
// Замер по счёту владельца: 396 постов за сутки обошлись примерно в $1, то
// есть ~$0.0025 за пост. При таком порядке цены прежний потолок в 400/сутки
// стоил доллар и при этом ограничивал сбор — соотношение бессмысленное.
// Новые пределы: ~$3.75 в сутки и жёсткий месячный стоп около $50.
/** Сколько постов в месяц разрешено прочитать (запас к тарифу). Экспортируется,
 * чтобы тесты проверяли ПОВЕДЕНИЕ, а не дублировали число: продублированная
 * константа расходится с кодом при первом же изменении лимита. */
export const MONTHLY_POST_BUDGET = Number(process.env.X_MONTHLY_POST_BUDGET ?? 20_000);
/**
 ...
 */
export const DAILY_POST_BUDGET = Number(process.env.X_DAILY_POST_BUDGET ?? 1500);
